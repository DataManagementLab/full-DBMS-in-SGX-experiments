...
def main():
    latencies, throughputs = load_throughputs_latencies("exp-3-results-plain", "exp-3-results-direct",
                                                        "exp-3-results-sgx")

    sns.barplot(latencies, x="Query", y="Latency", hue="Mode")
    plt.xticks(rotation=90)
    plt.tight_layout()
    plt.savefig("img/exp-3-latencies.pdf")
    plt.close()

    sns.barplot(throughputs, x="Query", y="Throughput", hue="Mode")
    plt.xticks(rotation=90)
    plt.tight_layout()
    plt.savefig("img/exp-3-throughput.pdf")
    plt.close()
